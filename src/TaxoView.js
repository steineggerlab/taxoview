...
export default function TaxoView() {
	...
	function filterData(allData) {
		...
		return allData.filter((entry) => {
			return config.minThresholdMode === 1 // 1 - proportion, 0 - clade_reads
				? parseFloat(entry.proportion) >= config.minThreshold
				: parseFloat(entry.clade_reads) >= config.minThreshold;
		});
	}
			...
			.forEach((rank) => {
				if (nodesByRank[rank]) {
					...
					const topNodes = nodesByRank[rank]
						...
						.slice(
							0,
							isFullGraph ? nodesByRank[rank].length : config.taxaLimit,
						);
		...
		function generateLinks(nodes, targetArray, sankeyRankColumns) {
			nodes.forEach((node) => {
				...
				while (previousNode) {
					...
					if (
						sankeyRankColumns.includes(previousNode.rank) &&
	...
	function highlightNodes(query) {
		...
		svg
			.selectAll("rect")
			.style("opacity", (d) =>
				config.searchQueryMatchNodes.has(d.id)
					? 1
					: config.lowlightShapeOpacity,
			);
		svg.selectAll("path").style("opacity", config.lowlightShapeOpacity);
		svg
			.selectAll("text.node")
			.style("opacity", (d) =>
				config.searchQueryMatchNodes.has(d.id) ? 1 : config.lowlightTextOpacity,
			);
		svg
	...
	function createSankey(fileContent) {
		...
		const ranksPresent = Array.from(new Set(nodes.map((n) => n.rank))).filter(
			(r) => rankListWithRoot.includes(r),
		);
		...
		const columnWidth =
			(config.width - config.marginRight) / config.ranksToShow.length;
		...
		graph.nodes.forEach((node) => {
			...
			node.color = node.isUnclassifiedNode
				? config.unclassifiedLabelColor
				: color(node.id);
		});
		...
		const highlightLineage = (node) => {
			...
			svg
				.selectAll("rect")
				.style("opacity", (d) =>
					lineageIds.has(d.id) ? 1 : config.lowlightShapeOpacity,
				);
			svg
				.selectAll(".link-path")
				.style("opacity", (d) =>
					lineageIds.has(d.source.id) && lineageIds.has(d.target.id)
						? config.linkPathOpacity
						: config.lowlightShapeOpacity,
				);
			svg
				.selectAll("text.node")
				.style("opacity", (d) =>
					lineageIds.has(d.id) ? 1 : config.lowlightTextOpacity,
				);
			svg
				.selectAll(".clade-reads")
				.style("opacity", (d) =>
		...
		nodeGroup
			...
			.attr(
				"fill",
				(d) =>
					`${d.isUnclassifiedNode ? config.unclassifiedLabelColor : d.color}`,
			);
		...
		nodeGroup
			...
			.attr(
				"fill",
				(d) =>
					`${d.isUnclassifiedNode ? config.unclassifiedLabelColor : config.fontFill}`,
			)
			.attr("font-weight", config.fontWeight)
			.attr("font-family", config.fontFamily)
